# author:oldpan
# data:2018-4-16
# Just for study and research

from __future__ import print_function
import argparse
import os

import cv2
import numpy as np
import torch

import torch.utils.data
from torch import nn, optim
from torch.autograd import Variable
from torch.nn import functional as F
import torch.backends.cudnn as cudnn

from models import Autoencoder, toTensor, var_to_np
from util import get_image_paths, load_images, stack_images
from training_data import get_training_data
# from torch.utils.tensorboard import SummaryWriter
'''7.设置训练网络的一些参数'''
parser = argparse.ArgumentParser(description='DeepFake-Pytorch')
parser.add_argument('--batch-size', type=int, default=64, metavar='N',
                    help='input batch size for training (default: 64)')
parser.add_argument('--epochs', type=int, default=23140, metavar='N',
                    help='number of epochs to train (default: 10000)')
parser.add_argument('--no-cuda', action='store_true', default=False,
                    help='enables CUDA training')
parser.add_argument('--seed', type=int, default=1, metavar='S',
                    help='random seed (default: 1)')
parser.add_argument('--log-interval', type=int, default=100, metavar='N',
                    help='how many batches to wait before logging training status')

args = parser.parse_args()
args.cuda = not args.no_cuda and torch.cuda.is_available()
if args.cuda is True:
    print('===> Using GPU to train')
else:
    print('===> Using CPU to train')

torch.manual_seed(args.seed)
if args.cuda:
    torch.cuda.manual_seed(args.seed)

print('===> Loaing datasets')

'''1.准备数据集''' 
images_A = get_image_paths("data/trump")
images_B = get_image_paths("data/cage")
images_A = load_images(images_A) / 255.0 # 像素值归一化

images_B = load_images(images_B) / 255.0 # 像素值归一化
# 对齐两个数据集的均值，减少域差异
images_A += images_B.mean(axis=(0, 1, 2)) - images_A.mean(axis=(0, 1, 2)) # 批次，高度，宽度维度上的均值相加

'''2.模型构建'''
'''3.模型引用'''

model = Autoencoder()


print('===> Try resume from checkpoint')
if os.path.isdir('checkpoint'): # 检查点恢复机制
    try:
        checkpoint = torch.load('./checkpoint/autoencoder.t7')
        model.load_state_dict(checkpoint['state'])
        start_epoch = checkpoint['epoch']
        print('===> Load last checkpoint data')
    except FileNotFoundError:
        print('Can\'t found autoencoder.t7')
else:
    start_epoch = 0
    print('===> Start from scratch')

if args.cuda:
    model.cuda()
    cudnn.benchmark = True
'''4.损失函数'''
criterion = nn.L1Loss()
'''5.优化器'''
optimizer_1 = optim.Adam([{'params': model.encoder.parameters()}, # model.encoder.parameters()：获取模型中 encoder（编码器）部分的所有可学习参数（如权重、偏置等）。
                          {'params': model.decoder_A.parameters()}] # model.decoder_A.parameters()：获取模型中 decoder_A（解码器A）部分的所有可学习参数。
                           , lr=5e-5, betas=(0.5, 0.999))
optimizer_2 = optim.Adam([{'params': model.encoder.parameters()},   # 同理
                          {'params': model.decoder_B.parameters()}]
                         , lr=5e-5, betas=(0.5, 0.999))

# print all the parameters im model
# s = sum([np.prod(list(p.size())) for p in model.parameters()])
# print('Number of params: %d' % s)
    
'''6.训练与测试网络'''
if __name__ == "__main__":

    print('Start training, press \'q\' to stop')

  #  writer = SummaryWriter("./log")

    for epoch in range(start_epoch, args.epochs):
        batch_size = args.batch_size
        '''数据加载'''
        warped_A, target_A = get_training_data(images_A, batch_size)
        warped_B, target_B = get_training_data(images_B, batch_size)

        warped_A, target_A = toTensor(warped_A), toTensor(target_A)
        warped_B, target_B = toTensor(warped_B), toTensor(target_B)

        if args.cuda:
            warped_A = warped_A.cuda()
            target_A = target_A.cuda()
            warped_B = warped_B.cuda()
            target_B = target_B.cuda()

        warped_A, target_A, warped_B, target_B = Variable(warped_A.float()), Variable(target_A.float()), \
                                                 Variable(warped_B.float()), Variable(target_B.float())
        '''
        以上是将输入数据转换为浮点型，然后用variable包装起来，使其支持自动求导，为后续loss计算与反向做准备

        # 转换为Tensor并设置数据类型为float（无需Variable）
        warped_A = toTensor(warped_A).float()
        target_A = toTensor(target_A).float()
        warped_B = toTensor(warped_B).float()
        target_B = toTensor(target_B).float()
        '''
        optimizer_1.zero_grad()
        optimizer_2.zero_grad()

        warped_A = model(warped_A, 'A')
        warped_B = model(warped_B, 'B')

        loss1 = criterion(warped_A, target_A)
        loss2 = criterion(warped_B, target_B)

        loss1.backward()
        loss2.backward()
        
        optimizer_1.step()
        optimizer_2.step()

        print('epoch: {}, lossA:{}, lossB:{}'.format(epoch, loss1.item(), loss2.item()))
        # tensorboard的使用
        #writer.add_scalar('loss1', loss1, epoch)
        #writer.add_scalar('loss2', loss2, epoch)
        #writer.add_graph(model, warped_A)
        #writer.add_graph(model, warped_B)

        # 后续的模型保存和可视化代码
        # 测试
        if epoch % args.log_interval == 0:
            with torch.no_grad():# 测试数据无需求导，可使用with torch.no_grad()提升效率
                # [start:end] py中的切片操作，选取从索引start,到索引end-1  如果end不写 就是要到最后一个元素
                # 这里的[0:14] 就是选取第0个到第13个元素
                test_A_ = target_A[0:14] 
                test_B_ = target_B[0:14]
                test_A = var_to_np(target_A[0:14])
                test_B = var_to_np(target_B[0:14])

            print('===> Saving models...')
            state = {
                'state': model.state_dict(),
                'epoch': epoch
            }
            if not os.path.isdir('checkpoint'):
                os.mkdir('checkpoint')
            torch.save(state, './checkpoint/autoencoder.t7')
            #torch.save(model.state_dict(), "autoencoder.pth")


        # 可视化结果
        with torch.no_grad():  # 推理时关闭求导，节省内存
            figure_A = np.stack([ # 川普-》卡哥
                test_A,
                var_to_np(model(test_A_, 'A')),
                var_to_np(model(test_A_, 'B')),
            ], axis=1)
            figure_B = np.stack([ # 卡哥-》川普
                test_B,
                var_to_np(model(test_B_, 'B')),
                var_to_np(model(test_B_, 'A')),
            ], axis=1)

        figure = np.concatenate([figure_A, figure_B], axis=0)
        figure = figure.transpose((0, 1, 3, 4, 2))
        figure = figure.reshape((4, 7) + figure.shape[1:]) # 4行

        figure = stack_images(figure)

        figure = np.clip(figure * 255, 0, 255).astype('uint8')

        cv2.imshow("", figure)
        key = cv2.waitKey(1)
        if key == ord('q'):
            exit()
 #   writer.close()